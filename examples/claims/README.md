...
This folder contains synthetic input PDFs and hand-written claim output.
...
requiring Azure, Snowflake, or Chroma. Research tests load the output as
read-only input.
...
The sample manifest uses lexical retrieval, so the fixture does not require a
Snowflake session or Chroma index.
