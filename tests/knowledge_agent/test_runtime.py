...
def test_browser_credential_factory_is_explicit(monkeypatch):
    ...
    monkeypatch.setattr(
        "knowledge_agent.infrastructure.runtime.InteractiveBrowserCredential",
        lambda: credential,
    )
...
def test_factory_builds_openrouter_client_and_closes_it(monkeypatch):
    ...
    monkeypatch.setattr("knowledge_agent.infrastructure.runtime.OpenAI", build_client)
...
def test_factory_builds_azure_client_with_browser_credential(monkeypatch):
    ...
    monkeypatch.setattr(
        "knowledge_agent.infrastructure.runtime.create_browser_credential",
        lambda: credential,
    )
    monkeypatch.setattr(
        "knowledge_agent.infrastructure.runtime.AIProjectClient",
        build_project,
    )
