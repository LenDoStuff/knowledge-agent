...
def set_llm_settings(monkeypatch, settings):
    monkeypatch.setattr(
        "ingest.bootstrap.LlmSettings.from_env",
...
def test_home_builds_key_ocr_without_snowflake_or_chroma(monkeypatch):
    ...
    monkeypatch.setattr(
        "ingest.bootstrap.create_openai_runtime",
        lambda settings: runtime,
    )
    monkeypatch.setattr(
        "ingest.bootstrap.AzureDocumentIntelligenceOcrClient",
        lambda endpoint, credential: ocr_calls.append((endpoint, credential)) or ocr,
    )
    monkeypatch.setattr(
        "ingest.bootstrap.SnowflakeAiEmbedder",
