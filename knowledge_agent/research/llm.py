...
class ResponsesResearchModel:
    ...
    def plan_queries(
        ...
        user = (
            ...
            f"Create up to {queries_per_question} distinct lexical search "
            "queries. Each query should target evidence needed to answer the "
            f"question.\n\nAvailable documents:\n{document_context}"
        )
        plan = self._parse(
    ...
    def extract_findings(
        ...
        user = (
            ...
            "Extract relevant findings. Each finding must include one or more "
            "exact source references that support it. Add focused follow-up "
            "questions only when more claim evidence is needed.\n\n"
            f"Evidence:\n{evidence_context}"
        )
        result = self._parse(
    ...
    def write_answer(
        ...
        user = (
            ...
            "Write a concise answer. If there are no findings, state that the "
            "claim knowledge base does not contain enough evidence."
        )
        result = self._parse(
